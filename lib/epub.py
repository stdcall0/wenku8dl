...
class Book:
  def __init__(self, meta: dict[str], book_id, title, chapter_cnt):
    ...
    if options['moreAuthor'] and 'contributor' in meta: self.book.add_metadata('DC', 'contributor', meta['contributor'])
    if options['moreAuthor'] and 'publisher' in meta: self.book.add_metadata('DC', 'publisher', meta['publisher'])
  ...
  def finalize(self, filename: str, cover_data, cover_ext: str):
    ...
    toc = [ENTRY('cover.xhtml', -1, '封面'), ENTRY('title.xhtml', -1, '标题'), ENTRY('makerinfo.xhtml', -1, '制作信息'), ENTRY('contents.xhtml', -1, '目录')]
    ...
    self.book.toc = [LINK('cover', '封面'), LINK('title', '标题'), LINK('makerinfo', '制作信息'), LINK('contents', '目录')] + extra_toc
    self.book.spine = [self.coverPage, self.titlePage, self.makerPage, self.tocPage] + extra_spine
